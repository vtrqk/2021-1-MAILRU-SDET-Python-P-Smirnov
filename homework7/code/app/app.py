...
def get_user_id_by_name(name):
    if user_id := app_data.get(name):
        ...
        my_mock_host = os.environ['MY_MOCK_HOST']
        my_mock_port = os.environ['MY_MOCK_PORT']

        job = None

        try:
            resp = requests.get(f'http://{my_mock_host}:{my_mock_port}/get_job/{name}')
            if resp.status_code == 200:
                job = resp.json()
        except Exception as e:
            print(f'Unable to get job from external system:\n{e}')
